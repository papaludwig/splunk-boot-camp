...
### Add Data Manually
Let's start by downloading the data. Splunk can ingest either the zipped archive or the uncompressed CSV directly. If your browser or security tooling blocks ZIP downloads, use the direct CSV link instead. You'll upload public data for consumer and visitor insights for neighborhoods from the US Census Bureau's census block group (CBG).
...
- [ZIP download](../data/visit-patterns-by-census-block-group.zip) the `visit-patterns-by-census-block-group.zip` file
- [Direct CSV download](https://github.com/papaludwig/splunk-boot-camp/raw/master/data/cbg_patterns.csv) the `cbg_patterns.csv` file if ZIP downloads are blocked in your environment
